...
# First-time bootstrap (run once on fresh VPS as root)
ansible-playbook bootstrap.yml --ask-vault-pass -e "ansible_user=root" -e "ansible_port=22" --ask-pass
...
- **bootstrap.yml** — One-time playbook run as root on a fresh VPS. Creates `deploy` user, hardens SSH (port 2222, key-only), configures UFW/fail2ban, then hands off to site.yml.
...
- `inventory/group_vars/all.yml` — All non-secret configuration (domains, GHCR image references, image tags, DB names, user names).
- `inventory/group_vars/vault.yml` — Ansible Vault-encrypted secrets (IP, SSH port, passwords, tokens, GHCR credentials). Template at `vault.yml.example`.
- Vault variables are prefixed `vault_*` and referenced from `all.yml`.
